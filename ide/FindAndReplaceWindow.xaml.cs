...
namespace ide
{
    public partial class FindAndReplaceWindow : Window
    {
        ...
        public FindAndReplaceWindow(TextEditor editor, int tab)
        {
            InitializeComponent();
        ...
        {
            if (txtFind2.Text == string.Empty) return;
            if (MessageBox.Show("Ви впевнені, що хочете замінити всі входження \"" +
            txtFind2.Text + "\" на \"" + txtReplace.Text + "\"?",
                "Замінити всі", MessageBoxButton.OKCancel, MessageBoxImage.Question) == MessageBoxResult.OK)
            {
        ...
        public static void ShowForReplace(TextEditor editor)
        {
            if (theDialog == null)
            {
                theDialog = new FindAndReplaceWindow(editor, 1);
